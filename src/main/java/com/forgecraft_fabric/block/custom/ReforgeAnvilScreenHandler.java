package com.forgecraft_fabric.block.custom;

import com.forgecraft_fabric.ForgeCraft;
import com.forgecraft_fabric.block.entity.ReforgeAnvilBlockEntity;
import com.forgecraft_fabric.items.ForgeData;
import com.forgecraft_fabric.items.forge.ForgeTypeGetter;
import com.forgecraft_fabric.items.forge.ForgingHelper;
...
public class ReforgeAnvilScreenHandler extends ForgingScreenHandler {
    ...
    public final ReforgeAnvilBlockEntity blockEntity;
    public static World world;
    ScreenHandlerContext context;
    public void SetContext(ScreenHandlerContext screenHandlerContext) {
        this.context = screenHandlerContext;
    }
    public ReforgeAnvilScreenHandler(int syncId, PlayerInventory inventory, PacketByteBuf buf) {
        this(syncId, inventory, ScreenHandlerContext.EMPTY, inventory.player.getWorld().getBlockEntity(buf.readBlockPos()),
                new ArrayPropertyDelegate(2));
    }

    public ReforgeAnvilScreenHandler(int syncId, PlayerInventory inventory, ScreenHandlerContext context, BlockEntity blockEntity, PropertyDelegate arrayPropertyDelegate) {
        super(ModScreenHandlers.REFORGE_ANVIL_SCREEN_HANDLER, syncId, inventory, context);
        checkSize(((Inventory) blockEntity), 2);
        this.inventory = ((Inventory) blockEntity);
        inventory.onOpen(inventory.player);
        this.propertyDelegate = arrayPropertyDelegate;
        this.context = context;
        this.blockEntity = ((ReforgeAnvilBlockEntity) blockEntity);
    ...
    protected void onTakeOutput(PlayerEntity player, ItemStack stack) {
        ...
        blockEntity.getWorld().playSound(null, blockEntity.getPos(), SoundEvents.BLOCK_ANVIL_USE, SoundCategory.BLOCKS, 1.0f, 0.9f);
        for(int num=0; num< 10;num++) {
            blockEntity.getWorld().addParticle(ParticleTypes.LAVA, blockEntity.getPos().getX()+0.5, blockEntity.getPos().getY() + 1, blockEntity.getPos().getZ()+0.5, 0.0, 0.1, 0.0);
        }
    ...
    public void updateResult() {
        ...
        if (itemStack.isEmpty() || itemStack3.isEmpty()) {
            ...
            this.levelCost.set(0);
            ForgeCraft.LOGGER.info("Quit: Input Empty!");
        } else {
            ...
            boolean isForge = ForgeTypeGetter.itemList.containsKey(itemStack3.getItem());
            boolean isForge_input_1 = ForgeTypeGetter.itemList.containsKey(itemStack.getItem());
            ...
            if (!itemStack3.isEmpty()) {
                ...
                if(isForge_input_1) {
                    this.output.setStack(0, ItemStack.EMPTY);
                    this.levelCost.set(0);
                    ForgeCraft.LOGGER.info("Quit: No Forge Item");
                    return;
                }
                if(!itemStack.isEmpty() && isForge) {
                    ForgingHelper.Clear();
                    ForgingHelper.InitForge(this.input,this.output,itemStack2,levelCost);
                    itemStack2 = ForgingHelper.ForgeItem(itemStack2,itemStack3.getItem());
                    ForgeCraft.LOGGER.info("Forge Success");
                }
                if (itemStack2.isDamageable() && itemStack2.getItem().canRepair(itemStack, itemStack3)) {
                    ...
                    if (l <= 0) {
                        this.output.setStack(0, ItemStack.EMPTY);
                        this.levelCost.set(0);
                        ForgeCraft.LOGGER.info("Quit: Not Repairable");
                        return;
                    }
                    ...
                    if ((!bl && (!itemStack2.isOf(itemStack3.getItem()) || !itemStack2.isDamageable())) && !isForge) {
                        this.output.setStack(0, ItemStack.EMPTY);
                        this.levelCost.set(0);
                        ForgeCraft.LOGGER.info("Quit: Not Enhanceable");
                        return;
                    }
                    ...
                    if (bl3 && !bl2) {
                        this.output.setStack(0, ItemStack.EMPTY);
                        this.levelCost.set(0);
                        ForgeCraft.LOGGER.info("Quit: Enchantment max/limit");
                        return;
                    }
                }
            ...
            if(!isForge) this.levelCost.set(j + i);
            if (i <= 0 && !isForge) {
            ...
            ForgeCraft.LOGGER.info("Forge Test: Input " + itemStack2.isEmpty()+ ", " + itemStack.isEmpty() +", Out " + itemStack3.isEmpty() + ", XP: " + levelCost.get());
            this.output.setStack(0, itemStack2);
            //ForgeCraft.LOGGER.info("Forged Item: "+ ForgingHelper.GetItemType(itemStack2));
